...
class G1Deploy {
  ...
  public:
    ...
    bool CreatePolicyCommand() {
      ...
      // DEBUG: dump first 200 ticks of (raw IL action, mujoco q_target) to /tmp/q_target_dump.csv
      // for alignment audit. One-shot — closes itself after 200 rows.
      {
        static int dbg_tick = 0;
        static FILE* dbg_fp = nullptr;
        if (dbg_fp == nullptr && dbg_tick == 0) {
          dbg_fp = fopen("/tmp/q_target_dump.csv", "w");
          if (dbg_fp) {
            fprintf(dbg_fp, "tick");
            for (int j = 0; j < 29; ++j) fprintf(dbg_fp, ",a_il_%d", j);
            for (int j = 0; j < 29; ++j) fprintf(dbg_fp, ",q_mj_%d", j);
            fprintf(dbg_fp, "\n");
          }
        }
        if (dbg_fp && dbg_tick < 200) {
          fprintf(dbg_fp, "%d", dbg_tick);
          for (int j = 0; j < 29; ++j) fprintf(dbg_fp, ",%.6f", floatarr[j]);
          for (int j = 0; j < 29; ++j) fprintf(dbg_fp, ",%.6f", motor_command_tmp.q_target.at(j));
          fprintf(dbg_fp, "\n");
          if (++dbg_tick == 200) { fflush(dbg_fp); fclose(dbg_fp); dbg_fp = nullptr; }
        }
      }
      motor_command_buffer_.SetData(motor_command_tmp);
      return true;
