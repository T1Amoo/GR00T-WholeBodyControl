...
class SimLoopConfig(BaseConfig):
    ...
    simulate_23dof: bool = False
    """Virtually amputate 6 G1 joints (waist roll/pitch + L/R wrist pitch/yaw) so
    the 29-DoF policy sees a 23-DoF-like robot. Zeros these motors in published
    LowState, forces 0 PD torque on them, and locks them via high damping."""

    simulate_23dof_damping: float = 5000.0
    """Damping applied to the 6 missing joints when simulate_23dof is True.
    Acts as a soft fallback — the actual rigid lock comes from a per-step
    qpos/qvel hard reset in DefaultEnv._pin_missing_23dof_joints."""
