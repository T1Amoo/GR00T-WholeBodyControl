...
from gear_sonic.utils.joint_constants import (
    MISSING_23DOF_INDICES_MJCF as MISSING_23DOF_INDICES,
...
class DefaultEnv:
    ...
    def init_scene(self):
        ...
        self._missing_23dof_addrs = []  # list of (qpos_adr, dof_adr) tuples
        if self.config.get("SIMULATE_23DOF", False):
            self._lock_missing_23dof_joints()

    def _lock_missing_23dof_joints(self):
        """Make the 6 joints absent on 23-DoF G1 hardware behave as if welded.

        Belt-and-suspenders:
          1. Crank dof_damping (handles small velocities cheaply).
          2. Cache (qpos_adr, dof_adr) so sim_step can hard-reset qpos/qvel
             after every mj_step — damping alone cannot beat constant gravity
             torque, so without the hard pin the waist drifts forward."""
        damping = self.config.get("SIMULATE_23DOF_DAMPING", 5000.0)
        for name in MISSING_23DOF_JOINT_NAMES:
            joint_id = mujoco.mj_name2id(self.mj_model, mujoco.mjtObj.mjOBJ_JOINT, name)
            if joint_id == -1:
                print(f"[simulate_23dof] joint '{name}' not found, skipping")
                continue
            dof_adr = self.mj_model.jnt_dofadr[joint_id]
            qpos_adr = self.mj_model.jnt_qposadr[joint_id]
            self.mj_model.dof_damping[dof_adr] = damping
            self._missing_23dof_addrs.append((qpos_adr, dof_adr))
        print(
            f"[simulate_23dof] locked {len(self._missing_23dof_addrs)} missing joints "
            f"with damping={damping} + per-step qpos/qvel hard pin"
        )

    def _pin_missing_23dof_joints(self):
        """Hard-reset qpos/qvel of the 6 missing joints to their rest values.
        Called after mj_step so gravity / policy commands cannot move them."""
        for qpos_adr, dof_adr in self._missing_23dof_addrs:
            self.mj_data.qpos[qpos_adr] = self.mj_model.qpos0[qpos_adr]
    ...
    def compute_body_torques(self) -> np.ndarray:
        ...
        if self.unitree_bridge is not None and self.unitree_bridge.low_cmd:
            ...
            if getattr(self.unitree_bridge, "simulate_23dof", False):
                for i in MISSING_23DOF_INDICES:
                    body_torques[i] = 0.0
        return body_torques
    ...
    def sim_step(self):
        ...
        if self._missing_23dof_addrs:
            self._pin_missing_23dof_joints()
