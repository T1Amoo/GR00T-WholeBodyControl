...
from isaaclab.envs.mdp import joint_pos_rel as _stock_joint_pos_rel
from isaaclab.envs.mdp import joint_vel_rel as _stock_joint_vel_rel
from isaaclab.managers import ObservationGroupCfg as ObsGroup
from isaaclab.managers import SceneEntityCfg
from isaaclab.utils import configclass


def joint_pos_rel_masked(env, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """`joint_pos_rel` with the 6 hardware-absent joints (IL idx) zeroed.

    Used by the 23DoF training pipeline so the policy/critic see exactly what
    real 23DoF hardware reports for the missing motors (≈0 because they don't
    physically exist).
    """
...
def last_action_masked(env, action_name: str | None = None) -> torch.Tensor:
    """`last_action` with the 6 hardware-absent joints (IL idx) zeroed.

    The policy emits 29-d actions in IL order; on the 23DoF hardware variant
    the 6 missing motors are physics-pinned at default qpos and their entries
    are zeroed in the deploy cpp guard before being passed to the robot. Without
    masking the obs history, whatever the actor output on those 6 dims (random
    drift away from 0 in undertrained ckpts) feeds back into the next step's
    `last_action` history, contaminating the policy/critic input.
    """
...
def joint_pos_multi_future_select_joints_masked(
