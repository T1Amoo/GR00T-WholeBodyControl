...
class TrackingCommand(CommandTerm):
    ...
    def __init__(
        ...
        # H1 fix: cache GLOBAL motion id (index into _motion_data_keys) for static/idle
        # reset override. NOTE: motion_ids stored on the env are LOCAL indices into the
        # currently-loaded subset (_curr_motion_ids), not global indices. Adaptive
        # resampling can swap the loaded subset, so we resolve global→local at every
        # reset call (see _resample_command). Disabled when static_reset_prob == 0
        # or static_motion_name is None.
        self._static_motion_global_id: int | None = None
        if (
            getattr(self.cfg, "static_motion_name", None)
            and getattr(self.cfg, "static_reset_prob", 0.0) > 0.0
        ):
            keys = self.motion_lib._motion_data_keys  # noqa: SLF001
            matches = [i for i, k in enumerate(keys) if self.cfg.static_motion_name in str(k)]
            if not matches:
                raise ValueError(
                    f"[TrackingCommand] static_motion_name '{self.cfg.static_motion_name}' "
                    f"not found among {len(keys)} loaded motions."
                )
            self._static_motion_global_id = int(matches[0])
            print(  # noqa: T201
                f"[TrackingCommand] static_reset enabled: prob={self.cfg.static_reset_prob}, "
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        if len(env_ids) > 0:
            if self.is_evaluating:
                ...
                # H1 fix: force a fraction of envs onto the cached static/idle motion to cover
                # the "robot idle + reference idle" reset region the deploy policy hits on flat
                # ground. Only active in the standard sampling branch (not eval / paired /
                # multi-object), and only when the cfg knobs are set. We resolve global→local
                # here because adaptive resampling can swap _curr_motion_ids between resets.
                if (
                    self._static_motion_global_id is not None
                    and self.cfg.static_reset_prob > 0.0
                ):
                    curr_match = (
                        self.motion_lib._curr_motion_ids  # noqa: SLF001
                        == self._static_motion_global_id
                    ).nonzero(as_tuple=True)[0]
                    if len(curr_match) > 0:
                        static_local_id = int(curr_match[0].item())
                        static_mask = (
                            torch.rand(len(env_ids), device=self.device)
                            < self.cfg.static_reset_prob
                        )
                        if static_mask.any():
                            static_env_ids = env_ids[static_mask]
                            self.motion_ids[static_env_ids] = static_local_id
                            self.motion_start_time_steps[static_env_ids] = 0
                    # else: static motion not in current loaded subset (after adaptive
                    # resample); skip override this batch — uniform_sampling_rate will
                    # cycle it back into the loaded set within a few resamples.
...
class TrackingCommandCfg(CommandTermCfg):
    ...
    # H1 fix: force a fraction of envs at reset onto a static/idle motion (e.g. neutral_idle_loop)
    # so the reset distribution covers "robot idle + reference idle" — the OOD region the deploy
    # policy hits on flat ground. 0.0 = disabled (default behavior unchanged).
    static_reset_prob: float = 0.0
    # Substring of a motion_lib key (e.g. "neutral_idle_loop_001"). None = disabled.
    static_motion_name: str | None = None
