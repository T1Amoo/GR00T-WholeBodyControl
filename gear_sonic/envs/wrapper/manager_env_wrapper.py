...
class ManagerEnvWrapper:
    ...
    def step(self, actions):
        ...
        # 23DoF: zero the 6 hardware-absent joint commands BEFORE env.step so
        # action_manager.action (which action_rate_l2 reads) sees the masked
        ...
        # Per-term episode-step-at-trigger diagnostic. Tracks the elapsed step
        # within the current episode; when a termination term fires, log the
        # mean (and count) so we can see WHEN in the episode each term kills.
        # Reads termination_manager._term_dones, which is set in compute()
        # before auto-reset, so values reflect the pre-reset state.
        ...
        if done_mask.any() and hasattr(self.env, "termination_manager"):
            tm = self.env.termination_manager
            term_dones = tm._term_dones  # noqa: SLF001 — [num_envs, n_terms]
            for i, name in enumerate(tm.active_terms):
                fired = term_dones[:, i] & done_mask
                if fired.any():
                    extras["to_log"][f"term_step/{name}/mean"] = (
                        self._episode_step_count[fired].float().mean()
                    )
                    extras["to_log"][f"term_step/{name}/count"] = fired.sum().float()
            self._episode_step_count[done_mask] = 0
