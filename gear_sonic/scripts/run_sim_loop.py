...
def main(config: ArgsConfig):
    ...
    wbc_config["SIMULATE_23DOF"] = config.simulate_23dof
    wbc_config["SIMULATE_23DOF_DAMPING"] = config.simulate_23dof_damping
    if config.simulate_23dof:
        print(
            f"[run_sim_loop] simulate_23dof=True (damping={config.simulate_23dof_damping}) "
            "— 6 missing joints will be zeroed in LowState and locked in MuJoCo"
        )
