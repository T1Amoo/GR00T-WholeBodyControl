...
def create_manager_env(config, device, args_cli):
    ...
    # 23DoF physics-lock for hardware-absent joints (config-gated)
    missing_dofs_cfg = config.get("missing_dofs", None)
    ...
    if use_missing_dofs_lock:
        ...
        env_instance_cfg.config["missing_dofs"] = {"enabled": True}
    else:
