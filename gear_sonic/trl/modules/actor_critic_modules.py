...
class Actor(nn.Module):
    ...
    def __init__(
        ...
        # 23DoF: optional locked-action mask. When set, force μ=0 and σ=const
        # on the listed action dims at distribution construction time so PPO's
        # log_prob and KL on those dims are deterministic constants → zero
        # gradient flows back to the mu-layer's locked output rows and to the
        # corresponding log_std/std entries → no gradient pollution from
        # hardware-absent dims into the shared trunk. Mirrors the deploy cpp
        # guard semantically. Defaults to no-op for 29DoF training.
        ...
        if locked_il is not None and len(locked_il) > 0:
            ...
            self.locked_action_std_const = float(
                algo_config.get("missing_dofs_action_std", 1e-4)
            )
        else:
            self.locked_action_mask = None
            self.locked_action_std_const = None
    ...
    def update_distribution(
        ...
        if torch.any(torch.isnan(std)) or torch.any(std <= 0):
            ...
            std = torch.clamp(std, min=1e-6)
        std_combined = (mean * 0.0 + std).clamp(min=1e-6)
        # 23DoF: clamp locked dims to deterministic 0 with tiny constant σ.
        # masked_fill returns a new tensor → autograd flows through unmasked
        # dims only; locked dims become parameter-independent constants in the
        # log_prob computation, so their gradient on log_std/mu_layer rows is
        # exactly zero. See Actor.__init__ for the full rationale.
        if self.locked_action_mask is not None:
            mean = mean.masked_fill(self.locked_action_mask, 0.0)
            std_combined = std_combined.masked_fill(
                self.locked_action_mask, self.locked_action_std_const
            )
        self.distribution = Normal(mean, std_combined)
